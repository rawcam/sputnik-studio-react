...
export const SpecificationsListPage: React.FC = () => {
  ...
  const sortedSpecs = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'project': sorted.sort((a, b) => getProjectName(a.projectId).localeCompare(getProjectName(b.projectId))); break;
      case 'sum': sorted.sort((a, b) => b.totalSum - a.totalSum); break;
      case 'items': sorted.sort((a, b) => b.itemsCount - a.itemsCount); break;
        ...
        <div className="spec-toolbar-row">
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
            <div className="view-toggle" style={{ display: 'flex', gap: '8px', background: 'var(--spec-card-bg)', padding: '4px', borderRadius: '40px' }}>
              <button className={`view-btn ${viewMode === 'grid' ? 'active' : ''}`} onClick={() => setViewMode('grid')} style={{ padding: '6px 16px', borderRadius: '40px', border: 'none', cursor: 'pointer', background: viewMode === 'grid' ? '#2563eb' : 'transparent', color: viewMode === 'grid' ? 'white' : 'var(--spec-text-primary)' }}>
                <i className="fas fa-th"></i> Сетка
              </button>
              <button className={`view-btn ${viewMode === 'list' ? 'active' : ''}`} onClick={() => setViewMode('list')} style={{ padding: '6px 16px', borderRadius: '40px', border: 'none', cursor: 'pointer', background: viewMode === 'list' ? '#2563eb' : 'transparent', color: viewMode === 'list' ? 'white' : 'var(--spec-text-primary)' }}>
                <i className="fas fa-list"></i> Список
              </button>
            </div>
            <input type="text" placeholder="Поиск по названию..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--spec-border-light)', fontSize: '0.8rem', width: '200px', background: 'var(--spec-bg-solid)', color: 'var(--spec-text-primary)' }} />
            <select value={filterProject} onChange={e => setFilterProject(e.target.value)} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--spec-border-light)', fontSize: '0.8rem', background: 'var(--spec-bg-solid)', color: 'var(--spec-text-primary)' }}>
              <option value="all">Все проекты</option>
              <option value="null">Без проекта</option>
              {projects.map(p => (<option key={p.id} value={p.id}>{p.shortId} {p.name}</option>))}
            </select>
            <select value={sortBy} onChange={e => setSortBy(e.target.value as SortField)} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid var(--spec-border-light)', fontSize: '0.8rem', background: 'var(--spec-bg-solid)', color: 'var(--spec-text-primary)' }}>
      ...
      {viewMode === 'grid' ? (
        <div className="specs-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
          {sortedSpecs.map(spec => (
            <div key={spec.id} className="spec-card" onClick={() => handleOpen(spec.id)} style={{ background: 'var(--spec-bg-solid)', borderRadius: '20px', padding: '20px', cursor: 'pointer', transition: 'all 0.2s', border: '1px solid var(--spec-border-light)' }}
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '12px', justifyContent: 'flex-end' }}>
                <button className="spec-button" onClick={(e) => { e.stopPropagation(); handleOpen(spec.id); }} title="Открыть"><i className="fas fa-eye"></i></button>
                <button className="spec-button" onClick={(e) => { e.stopPropagation(); handleDuplicate(spec); }} title="Дублировать"><i className="fas fa-copy"></i></button>
                {spec.projectId && (<button className="spec-button" onClick={(e) => { e.stopPropagation(); handleUnlink(spec.id); }} title="Открепить"><i className="fas fa-unlink"></i></button>)}
                <button className="spec-button spec-button-danger" onClick={(e) => { e.stopPropagation(); handleDelete(spec.id); }} title="Удалить"><i className="fas fa-trash-alt"></i></button>
              </div>
            </div>
          ))}
          {sortedSpecs.length === 0 && (<div style={{ textAlign: 'center', padding: '40px', color: 'var(--spec-text-secondary)' }}><i className="fas fa-file-alt" style={{ fontSize: '3rem', opacity: 0.5 }}></i><p>Нет спецификаций. Создайте первую!</p></div>)}
        </div>
      ) : (
        <div className="spec-table-container" style={{ overflowX: 'auto' }}>
          <table className="spec-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={{ textAlign: 'left', padding: '12px' }}>Название</th><th style={{ textAlign: 'left', padding: '12px' }}>Проект</th><th style={{ textAlign: 'right', padding: '12px' }}>Сумма (руб)</th><th style={{ textAlign: 'center', padding: '12px' }}>Позиций</th><th style={{ textAlign: 'left', padding: '12px' }}>Дата создания</th><th style={{ textAlign: 'center', padding: '12px', width: '120px' }}>Действия</th></tr></thead>
            <tbody>
              {sortedSpecs.map(spec => (
              ...
              {sortedSpecs.length === 0 && (<tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--spec-text-secondary)' }}><i className="fas fa-file-alt" style={{ fontSize: '2rem', opacity: 0.5 }}></i><p>Нет спецификаций. Создайте первую!</p></td></tr>)}
            </tbody>
          </table>
        </div>
      )}

      {/* Модальное окно – исправлено: убран лишний blur, добавлен z-index, фон полупрозрачный */}
      {showModal && (
        <div style={{
          ...
          <div style={{
            background: 'var(--spec-bg-solid)', borderRadius: '24px', width: '90%', maxWidth: '500px',
            padding: '24px', boxShadow: '0 20px 35px -12px rgba(0,0,0,0.2)'
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="Например: Основное оборудование" autoFocus style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--spec-border-light)', borderRadius: '12px', fontSize: '0.85rem', background: 'var(--spec-bg-solid)', color: 'var(--spec-text-primary)' }} />
            </div>
            <div style={{ marginBottom: '16px' }}>
              ...
              <select value={newProjectId} onChange={e => setNewProjectId(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--spec-border-light)', borderRadius: '12px', fontSize: '0.85rem', background: 'var(--spec-bg-solid)', color: 'var(--spec-text-primary)' }}>
