...
interface DataRow {
  ...
  name: string;
  quantity: number;
  unit: string;import React, { useState, useEffect, useRef } from 'react';
import Sortable from 'sortablejs';
import * as XLSX from 'xlsx';
...
export const SpecificationPage: React.FC = () => {
  ...
  const computeTotals = () => {
    ...
    for (const row of rows) {
      if (row.type === 'data') {
        ...
        const discountRub = gross - net;
        totalGrossRub += gross;
        totalRub += net;
        totalDiscountRub += discountRub;
  ...
  const exportToExcel = () => {
    const sheetData: any[][] = [['Вендор', 'Артикул', 'Наименование', 'Кол-во', 'Ед.', 'Валюта', 'Цена за ед.', 'Скидка %', 'Сумма скидки (валюта)', 'Цена после (валюта)', 'Валовая сумма (руб)', 'Итого (руб)', 'Поставщик', 'Статус']];
    for (const row of rows) {
      if (row.type === 'section') {
            ...
            <tr>
              {['drag', 'checkbox', 'num', 'vendor', 'sku', 'name', 'qty', 'unit', 'currency', 'price', 'discount', 'discountAmount', 'priceAfter', 'grossRub', 'totalRub', 'supplier', 'status', 'actions'].map(col => (
                <th key={col} style={{ width: columnWidths[col], padding: '8px 6px', borderBottom: '1px solid #e5e7eb', background: '#f8fafc', fontWeight: 600, position: 'relative' }}>
                  ...
                  {col === 'discount' && 'Скидка %'}
                  {col === 'discountAmount' && 'Сумма скидки (валюта)'}
                      ...
                      <td colSpan={15} style={{ padding: '8px 6px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <div style={{ flex: 1, textAlign: 'center' }}>
                            <i className={`fas ${row.collapsed ? 'fa-plus-square' : 'fa-minus-square'}`} onClick={() => toggleSection(row.id)} style={{ cursor: 'pointer', marginRight: '8px' }}></i>
                            ...
                            <i className="fas fa-chart-line" onClick={() => toggleSectionTotals(row.id)} style={{ cursor: 'pointer', marginLeft: '12px', color: row.showTotals ? '#3b82f6' : '#9ca3af' }} title="Показать/скрыть итоги по разделу"></i>
                          </div>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => addDataRowAfterId(row.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '1rem' }} title="Добавить строку"><i className="fas fa-plus-circle"></i></button>
                            <button onClick={() => deleteRow(row.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '1rem' }} title="Удалить раздел"><i className="fas fa-trash-alt"></i></button>
                          </div>
                        </div>
                      </td>
                    </tr>
  ...
  discount: number;
  discountAmount: number;
  priceAfter: number;import React, { useState, useEffect, useRef } from 'react';
import Sortable from 'sortablejs';
import * as XLSX from 'xlsx';
...
interface DataRow {
  ...
  unit: string;
  currency: string;
  price: number;          // цена за единицу в валюте (без скидки)
  discount: number;      // скидка в процентах
  discountAmount: number; // сумма скидки в валюте (на единицу)
  priceAfter: number;     // цена после скидки в валюте (на единицу)
  supplier: string;
  status: string;
}
...
interface SectionRow {
  ...
  title: string;
  collapsed: boolean;
  showTotals: boolean;    // показывать ли промежуточные итоги по разделу
}
...
export const SpecificationPage: React.FC = () => {
  ...
  const getGrossRub = (row: DataRow) => {
    return row.price * row.quantity * getRate(row.currency);
  };

  // Сумма после скидки в рублях
  const getTotalRub = (row: DataRow) => {
    return (row.priceAfter || 0) * row.quantity * getRate(row.currency);
  };
  ...
  // Подсчёт итогов по конкретному разделу
  const getSectionTotals = (sectionId: number) => {
    let sectionGross = 0, sectionNet = 0, sectionQty = 0;
    let start = false;
    for (const row of rows) {
  ...
  // Операции с данными (без изменений, только переименованы для ясности)
  const addDataRowAfterId = (afterId: number) => {
    const index = rows.findIndex(r => r.id === afterId);
    if (index === -1) return;
    const newId = nextId;
  ...
  const exportToExcel = () => {
    const sheetData: any[][] = [['Вендор', 'Артикул', 'Наименование', 'Кол-во', 'Ед.', 'Валюта', 'Цена за ед.', 'Скидка %', 'Сумма скидки (валюта)', 'Цена после скидки (валюта)', 'Валовая сумма (руб)', 'Итого (руб)', 'Поставщик', 'Статус']];
    for (const row of rows) {
      if (row.type === 'section') {
        sheetData.push([`=== ${row.title} ===`, '', '', '', '', '', '', '', '', '', '', '', '', '']);
      } else {
...
export const SpecificationPage: React.FC = () => {
  ...
  const getTotalRub = (row: DataRow) => (row.priceAfter || 0) * (row.quantity || 0) * getRate(row.currency);
  const formatNumber = (num: number): string => Math.round(num).toLocaleString('ru-RU');
